 ...
 */
app.controller('sansimCtrl', ['$scope', '$http', '$q','$timeout','$sce', function ($scope, $http, $q,$timeout,$sce) {
        ...
        if ($scope.count <= 50) {
            $http.get(preUrl + "/search", {params: {search: search}})
                    .then(function (response) {
                    ...
                    $http.get(preUrl + "/search", {params: {search: search, p: pageNumber, captcha: $scope.captcha}})
                            .then(function (response) {
                                if (response != null && response != 'undefined' && response.status == 200) {
                                    ...
                                    if($scope.search.so.length>0){
                                        // $scope.formatMsisdnMark();
                                        // $scope.danhdau();
                                        $scope.formatMark();
                                    }
                                    if ($scope.page.items == 'undefined' || $scope.page.items.length == 0) {
...
/*end sap xep*/
    ...
    $scope.getListStringSearch=function () {
        ...
        for(var i=0;i<$scope.page.items.length;i++){
            ...
            /*cat bo tien to' 3 so dau voi 10 so, hoac 4 so dau voi 11 so*/
            // var suffixMsisdn='';
            // var length=$scope.page.items[i][8].length;
            // if(length==10){
            //     suffixMsisdn=$scope.page.items[i][8].substring(3);
            // }else if(length==11){
            //     suffixMsisdn=$scope.page.items[i][8].substring(4);
            // }
            $scope.timViTriChuoiSoV2($scope.page.items[i][8]);
            // $scope.timViTriChuoiSoV2(suffixMsisdn);
            //tang them 3 hoac 4 don vi cho vitrichuoi (viec nay tranh tim va highlight nhung chuoi xuat hien dau` so')
            // if($scope.listViTriChuoi.length>0){
            //     var plus=(length==10)?3:4;
            //     for(var n=0;n<$scope.listViTriChuoi.length;n++){
            //         $scope.listViTriChuoi[n].min=$scope.listViTriChuoi[n].min+plus;
            //         $scope.listViTriChuoi[n].max=$scope.listViTriChuoi[n].max+plus;
            //     }
            // }
